...
2. **巡逻挂机移动机制**
   - **动态动作序列 (自定路线走位)**：每个巡逻组内部可添加任意数量、任意顺序的方向动作（如“向右走 2秒 -> 停顿 1秒 -> 向左走 1秒 -> 向右走 2秒 -> 向左走 0.1秒”）。支持动态添加和删除步骤动作，满足各种复杂的地图挂机路线要求。
   - **向下兼容性**：支持完美向下兼容旧版本的配置文件或预设，导入时会自动将旧版的“左右时长”配置无缝转换成新的动作链，配置零丢失。
